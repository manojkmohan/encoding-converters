...
class Payyan:
    ...
    def Uni2Ascii(self):
        if self.input_filename:
            uni_file = codecs.open(
                self.input_filename, encoding='utf-8', errors='ignore')
        else:
            uni_file = codecs.open(
                sys.stdin, encoding='utf-8', errors='ignore')
        text = ""
        if self.output_filename:
            output_file = codecs.open(
                self.output_filename, encoding='utf-8', errors='ignore',  mode='w+')
        while 1:
    ...
    def Ascii2Uni(self):
        if self.pdf:
            ...
            if status:
                ...
                self.input_filename = os.path.splitext(
                    self.input_filename)[0] + ".txt"
        if self.input_filename:
            ascii_file = codecs.open(
                self.input_filename, encoding='utf-8', errors='ignore')
        else:
            ascii_file = codecs.open(
                sys.stdin, encoding='utf-8', errors='ignore')
    ...
    def LoadRules(self):
        ...
        rules_file = codecs. open(
            self.mapping_filename, encoding='utf-8', errors='ignore')
        while 1:
